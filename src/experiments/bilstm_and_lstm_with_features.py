...
texts = [sentence.text for sentence in debates.read_all_debates(sep_by_deb=False)] #debates
texts.extend([sentence.text for sentence in debates.read_all_speeches(sep_by_deb=False)]) #speeches
...
results = []
exp_num = '35000/bilstm_f_features'
...
for test_deb, test, val, train in data_sets:
    ...
    train_feat = feats.fit_transform(train)
    val_feat = feats.fit_transform(val)
    test_feat = feats.fit_transform(test)
    feat_size = train_feat.shape[1]

    model = models.bilstm(word_index, embedding_filepath, features = True, feat_size = feat_size, embedding_trainable = False, INPUT_LENGTH = MAX_SENTENCE_LENGTH, EMBEDDING_DIM = 300, LSTM_OUTPUT_DIM=600, dropout=0.3)
    ...
    epochs = 30
    model.fit([x_train, train_feat], y_train, validation_data=([x_val, val_feat], y_val), callbacks = callbacks_list, epochs=epochs, verbose=2, batch_size=16)
    model.save(join('results', exp_num, 'model.h5'))

    # predict        
    model.load_weights(join('results', exp_num, str(test_deb) + 'weights.best.hdf5'))
    predictions = model.predict_classes([x_test, test_feat])
    pred_probs = model.predict([x_test, test_feat])
...
for test_deb, test, val, train in data_sets:
    ...
    model = models.lstm(word_index, embedding_filepath, features = True, feat_size = feat_size, embedding_trainable = False, INPUT_LENGTH = MAX_SENTENCE_LENGTH, EMBEDDING_DIM = 300, LSTM_OUTPUT_DIM=600, dropout=0.3)
